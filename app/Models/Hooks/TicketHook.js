...
const TicketHook = exports = module.exports = {}
const Alias = use('App/Alias/alias.json')

TicketHook.AnswerCreated = async modelInstance => {
...
TicketHook.fitKeys = async modelInstance => {
  const key = Object.keys(modelInstance.$attributes)
  // eslint-disable-next-line camelcase
  const where_key = {}
  Alias.ticket.map(value => {
    key.map(val => {
...
TicketHook.AnswerUpdate = async modelInstance => {
  Object.keys(modelInstance.$attributes).map((data) => {
...
TicketHook.fitKeysUpdate = async modelInstance => {
  const key = Object.keys(modelInstance.$attributes)
  // eslint-disable-next-line camelcase
  const where_key = {}
  Alias.ticket.map(value => {
