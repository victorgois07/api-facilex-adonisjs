...
const FlowChartHook = exports = module.exports = {}
const Alias = use('App/Alias/alias.json')

FlowChartHook.AnswerCreated = async modelInstance => {
...
FlowChartHook.fitKeys = async modelInstance => {
  const key = Object.keys(modelInstance.$attributes)
  // eslint-disable-next-line camelcase
  const where_key = {}
  Alias.flow_chart.map(value => {
    key.map(val => {
...
FlowChartHook.AnswerUpdate = async modelInstance => {
  Object.keys(modelInstance.$attributes).map((data) => {
...
FlowChartHook.fitKeysUpdate = async modelInstance => {
  const key = Object.keys(modelInstance.$attributes)
  // eslint-disable-next-line camelcase
  const where_key = {}
  Alias.flow_chart.map(value => {
